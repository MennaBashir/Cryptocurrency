...
function TableAccount(props) {
    ...
    const BuySelect = (eId) => {
        ...
        let Price = ele.getElementsByTagName("td")[5];
        if (Input.checked) {
        ...
        <>
            ...
            <div className="table1">
                <table className="table table-striped">
                    ...
                    <tbody>
                        {
                            DataAccounts.filter((ele) => state.Webdomain.toLowerCase() !== "" ? ele.Webdomain.toLowerCase().includes((state.Webdomain).toLowerCase())
                                : state.Seller.toLowerCase() !== "" ? ele.sellerText.toLowerCase().includes((state.Seller).toLowerCase())
                                    : state.Country.toLowerCase() !== "" ? ele.locationText.toLowerCase().includes((state.Country).toLowerCase())
                                        : state.Details.toLowerCase() !== "" ? ele.details.toLowerCase().includes((state.Details).toLowerCase())
                                            : state.Min !== 0 ? ele.price <= state.Min
                                                : state.Max !== 0 ? ele.price <= state.Max
                                                    : state.Id !== 0 ? ele.id == state.Id
                                                        : ele)
                                .map((el) => {
                                    if (el.category === props.setCategory) {
                                        return (
                                            <tr key={el.id} id={`${el.id}`}>
                                                ...
                                                <td>{el.details}</td>
                                                <td className="price1">{el.price}</td>
                                                ...
                                                <td><button className="btn btn-sm blue btn-primary" onClick={() => window.open("https://prnt.sc/j-55UigPkn9N", "_blank")}>View Proof</button></td>
