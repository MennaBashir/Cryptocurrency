...
function TapleShell() {
    ...
    const CheckWork = (e) => {
        ...
        let ele = a.getElementsByTagName("button")[3];
                                            ...
                                            : state.Min !== 0 ? ele.price <= state.Min
                                                : state.Max !== 0 ? ele.price <= state.Max
                                                    : state.Id !== 0 ? ele.id == state.Id
                                                        : ele)
                                .map((el) => {
                                    if (el.category === "shells") {
                                        return (
                                            <tr key={el.id} id={`${el.id}`}>
                                                <td><input type="checkbox" onChange={() => BuySelect(el.id)} /> </td>
                                                <td>{el.id}</td>
                                                <td>
                                                    <button className="btn btn-sm btn-primary " className={el.type === "linux" ? "blue" : "red"}>
                                                        <img src={el.hostImg} alt=".." />
                                                        {el.hostInfo}
                                                    </button>
                                                </td>
                                                <td>
