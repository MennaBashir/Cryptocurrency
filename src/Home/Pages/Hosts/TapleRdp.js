...
function TapleRdp() {
    ...
    const BuySelect = (eId) => {
        ...
        let Price = ele.getElementsByTagName("td")[11];
        if (Input.checked) {
                            ...
                            DataHost.filter((ele) => state.Host.toLowerCase() !== "" ? ele.hosting.toLowerCase().includes((state.Host).toLowerCase())
                                : state.Seller.toLowerCase() !== "" ? ele.sellerText.toLowerCase().includes((state.Seller).toLowerCase())
                                    : state.Country.toLowerCase() !== "" ? ele.locationText.toLowerCase().includes((state.Country).toLowerCase())
                                        : state.Window !== "" ? ele.windows === state.Window
                                    ...
                                    if (el.category === "rdp") {
                                        return (
                                            <tr key={el.id} id={`${el.id}`}>
                                                ...
                                                <td>{el.ip}</td>
                                                {el.source === "Created" ?
                                                    <td className="create"><img src={plus} alt=".." /> {el.source}</td> :
                                                    <td className="circle">
                                                        <img src={circle} alt="..." />
                                                    Pre Owned
                                            </td>
                                                ...
                                                <td>{el.access}</td>
                                                <td>{el.price}</td>
                                                <td><button className="btn btn-sm btn-primary Trans" onClick={(a)=>CheckBlackList(el.id)}>Check Blacklist</button></td>
